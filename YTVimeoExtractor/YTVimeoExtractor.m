...
-(void)fetchVideoWithVimeoURL:(NSString *)videoURL withReferer:(NSString *)referer isLive:(BOOL)isLive completionHandler:(void (^)(YTVimeoVideo * __nullable video, NSError * __nullable error))completionHandler{
    ...
    if (isValidURL == NO) {
        ...
        YTVimeoExtractorOperation *operation = [[YTVimeoExtractorOperation alloc]initWithURL:videoURL referer:referer isLive: isLive];
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Warc-retain-cycles"
    operation.completionBlock = ^{
        [[NSOperationQueue mainQueue]addOperationWithBlock:^{
